...
import { Lock, Navigation, CheckSquare } from 'lucide-react';
...
export default function StampBadge({ landmark, stamp, isActive, isLocked, onClick }: StampBadgeProps) {
  // Determine horizontal placement class for winding effect
  const getOffsetClass = (order: number) => {
    if (order === 1) return 'mx-auto justify-center';
    if (order === 2) return 'ml-auto justify-end pr-4';
    if (order === 3) return 'mr-auto justify-start pl-4';
    if (order === 4) return 'ml-auto justify-end pr-6';
    if (order === 5) return 'mr-auto justify-start pl-6';
    return 'mx-auto justify-center';
  };

  const offsetClass = getOffsetClass(landmark.order);

  // 1. Locked State
  if (isLocked) {
    return (
      <div className={`flex w-full ${offsetClass}`}>
        <div className="flex items-center gap-3 bg-[#1a1a1a]/5 p-2 pr-4 rounded-full border border-[#1a1a1a]/10 max-w-[240px] opacity-50 grayscale transition-all">
  ...
  if (stamp) {
    return (
      <div className={`flex w-full ${offsetClass}`}>
        <button 
          onClick={onClick}
          className="flex items-center gap-3 bg-[#CBA052]/10 hover:bg-[#CBA052]/20 p-2 pr-4 rounded-full border-2 border-[#CBA052]/40 max-w-[250px] transition-all duration-200 group focus:outline-none focus:ring-2 focus:ring-[#004225] text-left"
        >
